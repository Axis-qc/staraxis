 ...
 *
 * @description
 * LOD（层级细节）系统 - 统一管理渲染细节的层级切换与纹理优化。
 *
 * 作用：
 * - 根据相机缩放级别（zoom）和屏幕像素大小动态决定各类型实体的渲染方式。
 * - 恒星：zoom > 2,000 或屏幕像素 < 10px 时使用圆形指示纹理，否则加载真实纹理。
 * - 行星：屏幕像素 < 10px 时使用圆形指示纹理，zoom > 1,000 开始淡出，> 100,000 完全隐藏。
 * - 轨道：zoom > 1,000 时完全隐藏，不再计算。
 * - 网格：不参与LOD隐藏，始终显示。
 * - 六边形轮廓：始终显示，不隐藏。
 *
 * 核心设计：
 * - 动态纹理切换：基于屏幕像素大小而非固定zoom阈值，小天体更早切换为简化渲染。
 * - 预计算模式：在 buildFrameState 阶段一次性计算所有LOD决策。
 * - 快速剔除：当LOD判定完全隐藏时，直接跳过实体遍历，零开销。
 * - 渐隐效果：行星在 1,000-100,000 zoom 范围内线性淡出。
 *
 * @usage
 * - 在 worldRenderManager 中每帧调用 `computeLodState(zoom, options)`。
 * - 将结果存入 WorldFrameState.lod。
 * - 各渲染器从 frame.lod 查询状态，并通过屏幕像素计算决定是否使用真实纹理。
 *
 * @provides
 ...
 * - 各渲染器应只读取LOD状态，不自行计算zoom相关逻辑。
 * - 纹理切换决策由渲染器根据屏幕像素大小动态计算，而非完全依赖LOD系统。
 * - 选中实体不受LOD隐藏影响，始终渲染。
 */
...
export const DEFAULT_LOD_CONFIG: Required<LodOptions> = {
    enabled: true,
    star: {
        // zoom > 2_000 时使用圆形指示纹理（不再加载真实纹理）
        // zoom > 10_000 时进一步简化
        thresholds: [2_000, 10_000, 50_000, 100_000],
        allowHidden: false,  // 恒星始终显示（用圆形指示），不隐藏
